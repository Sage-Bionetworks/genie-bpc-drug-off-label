...
get_dmet_timing <- function(ca_ind_df) {
  ...
  # The above two cases are not logically exhaustive.  Let's check that they 
  #   are empirically.
  neither_case <- ca_ind_df %>%
    filter(stage_dx_iv %in% "Stage IV" & !is.na(dx_to_dmets_yrs))
  if (nrow(neither_case)) {
    print(glimpse(neither_case))
    cli::cli_alert_danger("Some rows don't meet our def of de novo or progressed")
  }
