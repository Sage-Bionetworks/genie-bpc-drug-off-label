...
gg_panc_cape <- dft_hdrug_cohort %>%
  ...
  plot_drug_use_times(
    ...
    plot_subtitle = "Blue = First approval, Magenta = Conference abstract"
  ) +
...
*Credit:* Thanks to Jennifer Hoppe for compiling and organizing all the NCCN guidelines.  This section was easy after that was done!

We discussed above the possibility that the NCCN guidelines could precede approvals and have an impact on the way people treat.  I thought a text parse of the PDFs, just counting up occurrences of Capecitabine, would be a reasonable way to start.

The following graph shows the number of mentions of "cape", which tend to be things like "Capecitabine" and "CapeOx".  I excluded "Capello", which is an author's name appearing in the references later on.
...
- We all know "number of mentions" is not a good metric - just a starting point so we can read versions where there's a big difference.  Here's some reading notes:
...
- Some of the very old guidelines didn't parse well, I'm not going to bother trying to fix since 2003 parsed OK with no mentions.
...
We observed several off label uses of Traztuzumab and Vemurafenib in CRC above.  One hypothesis was that these participants may have had a mutation in BRAF or ERBB2 which could explain the motivation for use.  The following table provides a flag for each one, ignoring timing entirely (in other words, a BRAF mutation at any time would result in `TRUE`).  `Record [reg]` is the record ID and regimen number.
