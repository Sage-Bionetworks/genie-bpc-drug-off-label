...
- Warner's `component` is mapped to BPC `cohort`.
- Warner's `condition` is mapped to BPC `agent`.  This is drug names from the `regimen_drugs` column.
...
- Mappings worthy of extra scrutiny:
  - agent = "Interferon" with no information about the specific protein, which is a common entry in BPC.  I mapped this to Interferon alfa-2b just to start the conversation.
  - Paclitaxel nanoparticle albumin-bound -> Nabpaclitaxel.  Correct?
  - Trastuzumab is usually "Trastuzumab Emtansine" or "Trastuzumb Deruxtecan" in the BPC data.  It's much more complicated in the indications sheet, so I did not assign a mapping.
...
prop_uses_covered <- dft_hdrug_ind %>%
  mutate(
...
- Of the `r nrow(dft_hdrug_ind)` uses of a drug (one person using a drug for one block of time), `r round(prop_uses_covered*100, 1)`% involve a drug which was successfully mapped to a drug in the indications sheet.



```{r}
dft_off_label_uses <- dft_hdrug_ind %>%
  group_by(cohort, agent) %>%
  summarize(
    n_uses = n(),
    n_indicated = sum(ind_exists, na.rm = T),
    prop_indicated = n_indicated / n_uses, # currently 0 or 1.
    .groups = "drop"
  ) %>%
  filter(prop_indicated %in% 0) %>%
  select(-c(n_indicated, prop_indicated))


dft_hdrug_ind_first_day <- dft_hdrug_fxd %>%
  filter_drugs_first_x_days(
    ., 
    x = 0, # within 0 days to start.
    var_index_time = 'dx_first_drug_use_int'
...
dft_off_label_uses_first_drug <- dft_hdrug_ind_first_day %>%
  group_by(cohort, agent) %>%
  summarize(
    n_uses = n(),
    n_indicated = sum(ind_exists, na.rm = T),
...
- The drug is started after the approval date.
- If the drug requires metastatic patient, the drug use started after a metastatic diagnosis.

The following table shows potential **Off-label** uses, which is just any use which doesn't meet the definition above.  For each {cohort, agent} combination we show the number of off-label uses:
...
The following table is the same as the previous one, except we limit to only the 
 first day of drug use by each person:
...
The following table shows a few relevant columns from the `indications.csv` sheet.  This is not filtered, so it contains approvals from other regulators.

```{r, include = T}
dft_ind_mapped %>% 
  select(component, regulator, date, condition, note, context, stage_or_status) %>%
  DT::datatable(.)
```
...
- Expand to checking for metastatic and date of approval.
