...
- Of the `r nrow(dft_hdrug_ind)` uses of a drug (one person using a drug for one block of time), `r round(prop_uses_covered*100, 3)`% involve a drug which was successfully mapped to a drug in the indications sheet.
...
dft_off_label_uses_first_drug <- dft_hdrug_ind_first_day %>%
  ...
  filter(agent %in% dft_hdrug_ind$agent) %>%
  group_by(cohort, agent) %>%
  summarize(
    n_uses = n(),
    n_indicated = sum(ind_exists, na.rm = T),
