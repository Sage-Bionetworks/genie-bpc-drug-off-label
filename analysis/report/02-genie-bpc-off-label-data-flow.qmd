...
```{r, include = T}
dft_flow_all %>%
  select(step, n_uses, n_drug, n_pts, prop_lost) %>%
...
Here is the drugs uses information over {cohort, step} in table form:
...
Another question was which drugs are lost when we restrict to those with a first approval after 1997.  Here is a full list for each cohort, showing the drugs that are lost *in that step only*:
...
Expansions which would take some time, and possibly require repairing code in other places:

- Reorganizing the order of restrictions to something more sensible, like restrictions on cases followed by restrictions on drugs.  The current order is mostly historical.
