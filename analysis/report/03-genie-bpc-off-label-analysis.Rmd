...
- **Chapter 1** - Cohort information.  This is now a separate report showing all the filtering steps we do.
- **Chapter 2** - Lines of therapy.  We dropped this because it was hard.

All this code lives on in a deprecated folder, but it's no longer a key part of the analysis.
...
dfp_olu_cohort_type %<>%
  flextable(.) %>%
  autofit(.) %>%
  flextable::color(i = nrow(dfp_olu_cohort_type), color = "gray50")
...
```{r}
#| eval: false
dft_non_mono <- dft_hdrug_determinations %>%
  filter(failure_type %in% "test_monotherapy") %>%
  select(cohort, record_id, ca_seq, regimen_number, drug_number) %>%
  left_join(
    .,
    dft_hdrug_conmed,
  ...
  select(cohort, record_id, ca_seq, regimen_number, drug_number, agent, drug_overlaps) %>%
  mutate(
    drug_overlaps = purrr::map_chr(
      .x = drug_overlaps,
...
dft_non_mono %>%
  count(cohort, agent, drug_overlaps)

```

### *R4.8 Monotherapy violations

Here are drug uses which were off label due to monotherapy requirement violations:

```{r}
#| include: true
#| eval: false
dft_non_mono %>%
  count(cohort, agent, drug_overlaps, name = "cases") %>%
  flextable(.) %>%
  autofit(.)
```
...
- We consider all drugs when looking for overlaps.  For example oxaliplatin is excluded based on approval date, but we bring it back in when looking at whether capecitabine was used as monotherapy.
- If we want to keep pushing on the `with` column we need to make this move of bringing in all drugs (for example, cisplatin is a common `with` entry).
...
```{r}
#| eval: false
agents_included <- unique(dft_hdrug_det$agent)

dft_ind_relevant <- dft_ind_mapped %>% 
  filter(regulator %in% "FDA") %>%
  filter(!is.na(mapped_cohort) & mapped_agent %in% agents_included) 

dft_ind_relevant <- dft_hdrug_det %>%
  count(cohort, agent, name = "n_uses") %>%
  rename(mapped_cohort = cohort, mapped_agent = agent) %>%
  left_join(
    dft_ind_relevant, ., by = c('mapped_cohort', 'mapped_agent')
  )

dft_with_expansions <- dft_ind_relevant %>% 
  filter(!is.na(with) & !(with %in% "not applicable")) %>% 
  group_by(with) %>%
  summarize(
    rough_relevance = sum(n_uses),
    .groups = 'drop'
  ) %>%
  arrange(desc(rough_relevance))
```


### *R4.9 "With" expansions

`with` from the indications sheet shows drugs that must be taken with the indication (or `0` for monotherapy).  In order to figure out what the most relevant entries are, we assign one point for each indication corresponding to one drug use (representing an indication we can evaluate).  This is called `rough_relevance` below:
...
- We will define taking a drug "with" another one as a definite overlap of one day or more.
- `rough_relevance` is rough, but it at least gets us to focus on drugs that are in our inclusion criteria and are used quite a few times.
...
dft_biomarker_expansions <- dft_ind_relevant %>% 
  filter(!is.na(biomarker) & !(biomarker %in% "not applicable")) %>% 
  group_by(biomarker) %>%
  summarize(
    rough_relevance = sum(n_uses),
    .groups = 'drop'
...
### *R4.10 Biomarker expansions

The same table as 4.9 above, but for biomarkers:

```{r}
#| include: true
dft_biomarker_expansions %>%
  dt_wrapper(.)
```

*Notes:*

- Breast cancer markers are an obvious first step.  To deal with unknowns it may make sense to only flag people who are confirmed to be NOT in the correct biomarker category.
- EGFR exon 19 makes sense d/t general interest.
...
```{r, include = T}
dft_ind_mapped %>% 
  select(component, regulator, date, condition, with,
