...
There were `r n_her2_exposure_fails` exposures in `r n_her2_people_fails` people which required HER2+, but we didn't find evidence of that in IHC/ISH values reported in pathology.  This is in Trastuzumab and Pertuzumab mostly.  In cases where no HER2 testing was done we declined to flag patients, only people with at least one normal test result were flagged.
...
- About a 50/50 split between people who are HER2+ and HER2-.  May need to plot out the path reports for these folks.
- 2 borderline results, which is about 8% of people.  The cohort overall is 1% borderline for this summary.  Not to make a mountain out of a duo.
...
dft_biom_breast_tests %>%
  filter(record_id %in% dft_biom_her2_fails$record_id) %>%
  filter(str_detect(test, "her2")) %>%
  group_by(record_id) %>%
  summarize(any_pos = any(biom_pos, na.rm = T), .groups = 'drop') %>%
  left_join(., 
            select(dft_biom_her2_fails, record_id, ca_bca_her_summ),
            by = c('record_id')
  ) %>%
  filter(!any_pos, ca_bca_her_summ %in% "Positive/elevated/amplified")
  # tabyl(any_pos, ca_bca_her_summ)

vec_olu_biom_her2 <- filter(
    dft_biom_her2_fails, ca_bca_her_summ %in% "Positive/elevated/amplified"
  )$record_id

dft_biom_her2_traj_tests <- dft_biom_breast_tests %>%
  filter(str_detect(test, "her2")) %>%
  filter(record_id %in% vec_olu_biom_her2) %>%
  select(record_id, event = test, dob_to_event = dob_path_proc_days, contains("biom"))
...
- Four of these people, who are all HER2+ based on the summary variables, have no path data to back that up at any time.
- The others seem to have taken trasztuzumab long before any HER2 interpretation pops up in the data.  Seems like missing path reports are a possibility.
...
- Lots of ERBB2 mutations/alterations in this group, about half.  Indicates (to me) that people may have been using NGS as evidence for administering trastuzumab.
- Based on 9/10 of the positive cases having altered ERBB2, we could infer that BPC curators also took this as evidence of people being HER2+ (recall that at least 4 of these people have no path data indicating positivity).
- GENIE attempts to remove germline data, ERBB2 is often germline mutated, so that's a major caveat here.
...
```{r}
vec_covered_bio_simple <- c() # leaving in case we get a biomarker declare script.
...
vec_covered_bio <- c(vec_covered_bio_simple, vec_covered_bio_complex)

n_checked_bio <- dft_poss_app %>%
  filter(ind_biomarker %in% vec_covered_bio) %>%
  nrow(.)

n_unchecked_bio <- dft_poss_app %>%
  filter(!(ind_biomarker %in% vec_covered_bio) & !is.na(ind_biomarker)) %>%
  nrow(.)

dfp_unchecked_bio <- dft_poss_app %>%
...
- `r n_checked_bio` have an "biomarker" entry we've covered in our checks.  
- `r n_unchecked_bio` have a "biomarker" entry we haven't covered yet.

Among those biomarker entries we haven't checked yet, `r sum(pull(dfp_unchecked_bio, n))` are part of a exposure-indication link (`n`) that is currently passing all tests.  Here's the list:
...
- Specific interests expressed:
  - EGFR deletions
  - ROS and RET fusions (fusions are tricky to get a denominator for in GENIE).
