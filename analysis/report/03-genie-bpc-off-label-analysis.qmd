...
**Obviously some major revisions needed on text here**
...
"Specific conditions not met" above is a catch-all for dozens of tests.  It's everything except having the correct cancer type (checked first) and having the correct date (checked last).  Things that we check include:

- Whether the patient is metastatic, iff required for the drug indication.
- Whether any required concomitant meds were used (including monotherapy, `test_with_ nothing`)
- *In progress* whether they met clinical conditions (HER2+, ALK mutation, etc.).

The following graph shows the number of cases considered off label as we add each test (from top to bottom).  While the test order does matter, a large horizontal jump when a test is added indicates that a valid indication could no longer be found for many people when we added that check.  That gives us a sign of something to pursue further below.
...
### R3.5 Expanded "Specific condidtions not met"
...
# we'll get back to this, just a quick add since Shawn asked.
dft_hdrug_conmed

dft_stepwise_new_olu %>% 
  filter(step %in% 'test_with_carbotaxol') %>% 
  pull(new_off_label) %>% 
  `[[`(.,1) %>%
  left_join(., dft_hdrug_conmed, 
            by = c('cohort', 'record_id', 'ca_seq', 'regimen_number',
                   'drug_number', 'agent')
  ) %>%
  count(cohort, agent, drug_overlaps_vec, sort = T) %>%
  View(.)
