...
gg_panc_cape <- dft_hdrug_cohort %>%
  ...
  geom_vline(xintercept = ymd("2022-12-14"), color = '#177bc2') +
...
```{r}
#| include: true
gg_panc_cape_and_all <- cowplot::plot_grid(
  gg_panc_cape,
  gg_panc_all_drug,
  ncol = 1
)

gg_panc_cape_and_all
```
...
gg_nsclc_ipili <- dft_hdrug_cohort %>%
  ...
  plot_drug_use_times(
    ...
    # removing this since it's in another graph.
    # plot_subtitle = "Blue = First FDA approval"
  ) +
...
ggsave(fig3,
       filename = here('output', 'manu', 'fig3', 'fig3-capecitabine-use-nccn.pdf'),
       height = 5,
       width = 7
)
```
...
**Note:** Tables are currently broken, will get to this.
...
- Exon 19 in-frame insertions were present (GENIE-MSK-P-0008602), as were missense alterations on exon 19 (p.L747A on GENIE-DFCI-168454), splice variants, structural variants (GENIE-MSK-P-0025284, but after osi started).  These generally have lower "evidence levels" on OncoKB, down to none at all.

This is a candidate for biomarker-driven use.  Lots of these people have weird EGFR things that someone seemed to think were worth trying.
