...
dfp_olu_type_cohort %<>%
  ...
  flextable(.) %>%
  add_header_row(
    values = c("", paste0("m=", vec_olu_denoms)),
    top = F
  ) %>%
  autofit(.) %>%
...
This table shows the high level results in each cohort, split by three broad categories.  Details on the checks going into each category are given in the next section.
...
n_checked <- dft_poss_app %>%
  filter(ind_with %in% vec_covered_with) %>%
  nrow(.)

n_unchecked <- dft_poss_app %>%
  filter(!(ind_with %in% vec_covered_with) & !is.na(ind_with)) %>%
  nrow(.)

dfp_unchecked <- dft_with_unchecked <- dft_poss_app %>%
...
- `r n_checked` have an "with" entry we've covered in our checks.  Not all the checks are optimal (for example, for "chemotherapy" we just check that at least one conmed was used).
- `r n_unchecked` have a "with" entry we haven't covered yet.

Among those "with" entries we haven't checked yet, `r sum(pull(dfp_unchecked, n))` are part of a exposure-indication link that is currently passing all tests.  Prioritizing those entries rows is the highest priority in my view.  Here's the list:
