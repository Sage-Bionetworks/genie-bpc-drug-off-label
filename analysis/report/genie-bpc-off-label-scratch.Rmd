...
test_case <- hdrug_all %>%
  filter(
    cohort %in% "BLADDER" & 
      record_id %in% 'GENIE-DFCI-091449' & 
      ca_seq %in% 1
  )

plot_drug <- function(dat_drug_sub) {
  if (unique(length(dat_drug_sub$record_id)) > 1) {
    cli::cli_alert_danger("Function only intended to work on one person.  More than one exists in the input data")
  }
  ...
  dat_drug_sub %<>%
    ...
    mutate(
      ...
      end_yr = dx_drug_end_or_lastadm_int / 365.25
    )
